...
class MarginSoftmaxLoss(TopVirtualLoss):
    ...
    def forward(self, inputs, targets):
        ...
        if self.mhe_loss:
            ...
            return self.loss_function(outputs/self.t, targets) + the_mhe_loss + self.ring_loss * ring_loss, prec1
        elif self.inter_loss > 0:
            return self.loss_function(outputs/self.t, targets) + self.inter_loss * inter_loss + self.ring_loss * ring_loss, prec1
        else:
            return self.loss_function(outputs/self.t, targets) + self.ring_loss * ring_loss, prec1
