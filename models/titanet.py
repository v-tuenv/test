...
import numpy as np
import models.modules_tita as modules
from models.pooling import ASP, SAP, MultiHeadAttentionPooling
...
class TitaNet(nn.Module):
    ...
    def get_n_params(self, div=1):
        ...
        """
        return (
            sum([np.prod(p.size()) for p in self.parameters() if p.requires_grad]) / div + (self.embeddig_size * self.output_size)/div
        )
